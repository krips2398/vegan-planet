...
## Number of restaurants in each meteropolitian city
```{r}
...
## Relation difficulty in finding the restaurants and trouble finding restaurants

```{r}
df <- read_csv("project_data/dicho_variables.csv")
ggplot(data = df %>% gather(Variable, Count, -ALLLENGTH), 
       aes(x = ALLLENGTH, y = Count, fill = Variable)) + 
